...
check_os() {
    if [[ -f /etc/os-release ]]; then
        ...
        OS="centos"; PKG_MGR="yum"; OS_DESC=$(cat /etc/redhat-release)
    else
...
input_domain() {
    ...
    while true; do
        ...
        echo ""
        printf "确认域名为：\033[0;36m%s\033[0m？(回车/y 确认，n 重新输入): " "$DOMAIN"
        read -r yn
        case "${yn:-y}" in
            [Yy]*|"") break ;;
            *) warn "请重新输入域名" ;;
...
install_cert() {
    ...
    while true; do
        ...
        ISSUE_OUT=$("$ACME" --issue -d "$DOMAIN" --standalone --keylength ec-256 2>&1)
        ISSUE_EXIT=$?
        echo "$ISSUE_OUT"

        # 证书已存在未到期 → acme.sh 跳过并返回非0，视为成功
        if echo "$ISSUE_OUT" | grep -qE "Skipping|Domains not changed"; then
            info "证书已存在且有效，跳过申请直接安装。"
            break
        fi
        ...
        warn "----------------------------------------------"
        echo ""
        printf "完成 DNS 解析后按回车重试，或输入 q 退出安装: "
        read -r retry_input
        [[ "${retry_input}" == "q" || "${retry_input}" == "Q" ]] && error "已退出安装"
        info "正在重试申请证书..."
    done

    # reloadcmd：先判断服务是否在运行再操作，避免首次安装时报错
    "$ACME" --install-cert -d "$DOMAIN" --ecc \
        --cert-file      "$CERT_DIR/cert.crt" \
        --key-file       "$CERT_DIR/private.key" \
        --fullchain-file "$CERT_DIR/fullchain.crt" \
        --reloadcmd      "systemctl is-active --quiet nginx && systemctl reload nginx; systemctl is-active --quiet trojan-go && systemctl restart trojan-go; true"
...
verify_install() {
    ...
    systemctl is-active --quiet trojan-go \
        && echo -e "  ${GREEN}✓${NC} Trojan-Go 运行正常（:$PORT）" \
    ...
    [[ "$HTTP_CODE" == "200" ]] \
        ...
        || echo -e "  ${YELLOW}!${NC} 伪装页 HTTP $HTTP_CODE"
}
